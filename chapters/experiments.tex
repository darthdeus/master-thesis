...
The informative prior also achieves slightly lower variance than the non-informative one, which matches our intuition of guiding the GP regression model into a more stable area of the kernel parameter space. We confirm this with a kernel parameter visualization as shown in \autoref{figure:informative-priors}, where we can clearly see the parameters to be more stable, than in the case of non-informative priors as shown in \autoref{figure:kernel-parameters-over-time-jumpy}.\todo{MS: Tomu porovnani nerozumim, protoze pokazde to jsou hyperparametry jine ulohy. Jestli chces ukazovat, ze informative prior pomaha, potreboval bys vizualiazaci kernel parametru pro non-informative prior.}
